// Configuration
const API_BASE_URL = 'http://localhost:5001'; // CurrentIt backend
...
async function handleConversion() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/rate?from=${from}&to=${to}`);
...
async function handleLoadRates() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/rate?from=USD&to=RWF`);
        ...
        for (const currency of currencies) {
            if (currency !== 'USD') {
                const res = await fetch(`${API_BASE_URL}/api/rate?from=USD&to=${currency}`);
...
async function handleLoadChart() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/history?from=${from}&to=${to}&days=7`);
        const data = await response.json();
